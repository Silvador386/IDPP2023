...
def merge_dfs(
    ...
    relapses_df = prep_df_for_merge(relapses_df, id_feature_name, ts_feats, oo_feats, sort_by_this_feat)
    ...
    ms_type_df = prep_df_for_merge(ms_type_df, id_feature_name, ts_feats, oo_feats, sort_by_this_feat)
    ...
    mri_df = prep_df_for_merge(mri_df, id_feature_name, ts_feats, oo_feats, sort_by_this_feat)
    ...
    evoked_p_df = prep_df_for_merge(evoked_p_df, id_feature_name, ts_feats, oo_feats, sort_by_this_feat)
    ...
    edss_df = prep_df_for_merge(edss_df, id_feature_name, ts_feats, oo_feats, sort_by_this_feat)
...
def merge_train_test_dataframes(
        multi_merge_dfs: list[pd.DataFrame],
        id_feature_name: str
) -> tuple[list[str], list[str], pd.DataFrame]:
    train_ids, test_ids = [merge_df[id_feature_name] for merge_df in multi_merge_dfs]
    new_merged_df = pd.concat(multi_merge_dfs, axis=0, ignore_index=True)
    return train_ids, test_ids, new_merged_df
